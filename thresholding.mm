...
void dynamicThresholding(const cv::Mat &src, cv::Mat &dst) {
  ...
  cv::Mat labels;
  cv::Mat centers;
  cv::kmeans(reshapedImage, clusterCount, labels,
             cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::COUNT,
                              10, 1.0),
             3, cv::KMEANS_PP_CENTERS, centers);

  // Calculate the average intensity of the two clusters
  float center1 = (centers.at<float>(0, 0) + centers.at<float>(0, 1) +
                   centers.at<float>(0, 2)) /
                  3.0f;
  float center2 = (centers.at<float>(1, 0) + centers.at<float>(1, 1) +
...
void thresholdImage(const cv::Mat &src, cv::Mat &dst, int thresholdValue) {
  ...
  dst = cv::Mat::zeros(src.rows, src.cols,
                       CV_8U); // Initialize with zeros (all black)
