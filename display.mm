...
void displayImages(const cv::Mat &original, const cv::Mat &thresholded,
  ...
  cv::moveWindow("Thresholded Image", width,
                 0); // move the window to the right of the original image
  ...
  cv::moveWindow("Morphological Image", 2 * width,
                 0); // move the window to the right of the thresholded image
  ...
  if (!segmented.empty()) {
    ...
    cv::moveWindow(
        "Segmented Image", 3 * width,
        0); // move the window to the right of the morphological image
    height = (int)(width * (segmented.rows / (float)segmented.cols));
    cv::resizeWindow("Segmented Image", width, height);
  } else {
    std::cerr << "Error: Segmented Image is empty and cannot be displayed."
              << std::endl;
  }
