...
std::vector<cv::Point> computeRegionContour(const cv::Mat &labels,
                                            int regionID) {
  std::vector<std::vector<cv::Point>> contours;
  cv::Mat regionMask;
  cv::compare(labels, regionID, regionMask, cv::CMP_EQ);
  cv::findContours(regionMask, contours, cv::RETR_EXTERNAL,
...
float hausdorffDistance(const std::vector<cv::Point> &contour1,
                        const std::vector<cv::Point> &contour2) {
  auto h = [](const std::vector<cv::Point> &A,
              const std::vector<cv::Point> &B) -> float {
    float maxDistA = 0.0f;
    for (const auto &a : A) {
